...
import com.example.moviedbapplication.model.remote.TrailerResponse
...
class MovieRepository @Inject constructor(private val apiService: ApiService) {
    ...
    suspend fun fetchMovieTrailers(movieId: Int): Resource<TrailerResponse> {
        return withContext(Dispatchers.IO) {
            try {
                val response =
                    apiService.getMovieTrailers(movieId = movieId, apiKey = Constants.API_KEY)
                if (response.isSuccessful) {
                    Resource.Success(response.body())
                } else {
                    Resource.Error("API Error: ${response.code()} ${response.message()}")
